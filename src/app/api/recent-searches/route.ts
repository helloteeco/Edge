...
// PUT - Bulk sync local searches to Supabase (called on sign-in or page load)
export async function PUT(request: NextRequest) {
  try {
    const clientIP = getClientIP(request);
    const rateLimitResult = await rateLimit(`recent-searches-bulk:${clientIP}`, RATE_LIMITS.standard);
    if (!rateLimitResult.success) {
      return NextResponse.json({ success: false, error: "Too many requests" }, { status: 429 });
    }

    const body = await request.json();
    const { email, searches } = body;

    if (!email || !Array.isArray(searches) || searches.length === 0) {
      return NextResponse.json({ success: false, error: "Email and searches array required" }, { status: 400 });
    }
        ...
        .upsert(
          {
            ...
            analyzed_at: search.timestamp ? new Date(search.timestamp).toISOString() : new Date().toISOString(),
    ...
    console.error("[RecentSearches] Bulk sync error:", error);
    return NextResponse.json({ success: false, error: "An error occurred" }, { status: 500 });
  }
}
