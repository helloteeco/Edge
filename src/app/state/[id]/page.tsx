    ...
    <div className="min-h-screen pb-24" style={{ backgroundColor: '#e5e3da' }}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div 
          ...
          style={{ backgroundColor: '#2b2823', border: '1px solid #2b2823' }}
        >
          <div className="flex items-center justify-between mb-3">
            <div>
              <div className="flex items-center gap-2">
                <span className="inline-block w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>
                <span className="text-sm font-semibold" style={{ color: '#ffffff' }}>Live Market Data</span>
              </div>
              <p className="text-xs mt-1" style={{ color: '#9a9488' }}>Powered by Mashvisor • Real-time STR metrics</p>
            </div>
            {!showLiveData && (
              <button
                onClick={fetchLiveCities}
            ...
            <div className="mt-4">
              {loadingCities ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-sm" style={{ color: '#9a9488' }}>Loading live data...</p>
                </div>
              ) : liveCities.length > 0 ? (
                <div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {liveCities.map((city) => (
                      <button
                        key={city.name}
                        onClick={() => fetchNeighborhoods(city.name)}
                        className="px-3 py-2 rounded-lg text-sm font-medium transition-all"
                        style={{ 
                          backgroundColor: selectedCity === city.name ? '#ffffff' : 'rgba(255,255,255,0.1)',
                          color: selectedCity === city.name ? '#2b2823' : '#ffffff'
                        }}
                      >
                        {city.name}
                        <span className="ml-1 text-xs opacity-70">{city.occupancy}%</span>
                      </button>
                    ))}
                  </div>
                  
                  {/* Selected City Stats */}
                  {selectedCity && liveCities.find(c => c.name === selectedCity) && (
                    <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
                      <h4 className="font-semibold mb-3" style={{ color: '#ffffff' }}>{selectedCity} Overview</h4>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {(() => {
                          const city = liveCities.find(c => c.name === selectedCity)!;
                          return [
                            { label: 'Occupancy', value: `${city.occupancy}%` },
                            { label: 'Avg ADR', value: `$${city.adr}` },
                            { label: 'Monthly Rev', value: `$${city.monthlyRevenue?.toLocaleString()}` },
                            { label: 'STR Cap Rate', value: `${city.strCapRate}%` },
                          ].map(stat => (
                            <div key={stat.label}>
                              <div className="text-xs" style={{ color: '#9a9488' }}>{stat.label}</div>
                  ...
                  {selectedCity && (
                    <div>
                      <h4 className="font-semibold mb-3" style={{ color: '#ffffff' }}>Neighborhoods in {selectedCity}</h4>
                      {loadingNeighborhoods ? (
                        <div className="text-center py-4">
                          <div className="animate-spin w-6 h-6 border-2 border-white border-t-transparent rounded-full mx-auto"></div>
                        </div>
                      ) : liveNeighborhoods.length > 0 ? (
                        <div className="space-y-2 max-h-80 overflow-y-auto">
                          {liveNeighborhoods.map((nb) => (
                            <div 
                              key={nb.id}
                            ...
                            >
                              <div>
                                <div className="font-medium text-sm" style={{ color: '#ffffff' }}>{nb.name}</div>
                                <div className="text-xs" style={{ color: '#9a9488' }}>
                                  {nb.listingsCount} listings • ${nb.adr}/night
                                </div>
                              </div>
                              <div className="text-right">
                                <div className="text-lg font-bold" style={{ color: '#22c55e' }}>{nb.occupancy}%</div>
                        ...
                        <p className="text-sm" style={{ color: '#9a9488' }}>No neighborhood data available</p>
                      )}
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-sm" style={{ color: '#9a9488' }}>No live data available for this state</p>
              )}
            </div>
          )}
        </div>
