#!/usr/bin/env python3
"""
Append new cities to existing city-data.ts file by state - Fixed version
"""
import json
import re
import random
...
for state, cities in cities_by_state.items():
    ...
    if match:
        ...
        # The state_content should end with a city entry ending in },
        # We just append our new entries before the closing bracket
        # Make sure the last existing entry has a comma
        state_content_stripped = state_content.rstrip()
        if state_content_stripped and not state_content_stripped.endswith(','):
